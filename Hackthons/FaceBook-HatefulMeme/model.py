...
class DecoderLayer(nn.Module):
    def __init__(self,edim, nhead, dim_feedforward, dropout):
        ...
        self.multihead_attn = nn.MultiheadAttention(edim, nhead, dropout=dropout)
    ...
    def forward(self,src,mem,tgt_mask=None, memory_mask=None,
        ...
        src2 = self.multihead_attn(query,key,value,key_padding_mask=tgt_key_padding_mask)[0]
...
class HMModel(nn.Module):
    def __init__(self, edim):
        ...
        self.decoder = nn.TransformerDecoder(decoder_layer=self.decoderLayer,num_layers=4)
    def forward(self,image,input_ids,attention_mask):
        resnet_out = self.resnest(image)
        bert_out = self.roberta(input_ids=input_ids,attention_mask=attention_mask)[0]
        ...
        out = self.decoder(tgt = resnet_out, memory = bert_out,tgt_key_padding_mask=attention_mask)
        return out

# %%
model = HMModel(512)
loss_fn = nn.BCEWithLogitsLoss().cuda() if torch.cuda.is_available() else nn.BCEWithLogitsLoss()
#%%
...
print("Trainable parametes:", sum(p.numel() for p in model.parameters() if p.requires_grad))
...
def training(train_loader,num_epoch,model,loss_fn,optimizer,train_length,val_length):
    ...
    for epoch in range(1,num_epoch+1):
        ...
        for image,input_ids,attention_mask,target in train_loader:
            ...
            if torch.cuda.is_available():
                plabel = torch.where(preds>0.5, torch.Tensor([1.0]).cuda(),torch.Tensor([0.0]).cuda())
            else:
                plabel = torch.where(preds>0.5, torch.Tensor([1.0]),torch.Tensor([0.0]))
        ...
        if epoch ==1 or epoch%1==0:
            ...
            print('{} Epoch {}, Train Loss {}, Train Acc {}'.format(datetime.datetime.now(), epoch, epoch_train_loss,epoch_train_acc))
        ...
        with torch.no_grad():
            ...
            print('{}, Val Loss {}, Val Acc {}'.format(datetime.datetime.now(),epoch_val_loss,epoch_val_acc))
