...
# %%
# %%
model = SentimentClassifier(4).cuda() if torch.cuda.is_available() else SentimentClassifier(4)
...
def training(train_loader,num_epoch,model,loss_fn,optimizer,train_length,val_loader,val_length):
    ...
    for epoch in range(1,num_epoch+1):
        ...
        if epoch ==1 or epoch%1==0:
            ...
            print('{} Epoch {}, Train Loss {}'.format(datetime.datetime.now(), epoch, epoch_train_loss))
        model = model.eval()
...
optimizer = torch.optim.AdamW([{'params' : model.bert.parameters(),'lr':0.00001},{'params': model.linear.parameters(),'lr':0.1}])
training(train_loader,5,model,loss_fn,optimizer,train_length,val_loader,val_length)
# %%
test_loader = create_Dataloader(text=test_data,targets=None,tokenizer=tokenizer,max_length=64,batch_size=128, infer=True)
# %%
train_length = len(train_loader)
