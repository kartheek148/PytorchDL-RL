...
class Bandit(object):
    ...
    def update_est_value(self, action):
        ...
        self.est_values[action] = (self.est_values[action] + (
            1/self.action_sample_count[action])*(reward - self.est_values[action])).item()
...
class NonStationary(Epsgreedy):
    ...
    def sampling(self, step):
        self.true_values = normal.Normal(
            0.0, (step-1)*0.01).sample([self.num_actions])
...
class ConstantStep(NonStationary):
    ...
    def update_est_value(self, action):
        reward = self.get_reward(action)
        self.action_sample_count[action] += 1.0
        self.est_values[action] = (
            self.est_values[action] + (self.step_size)*(reward - self.est_values[action])).item()
...
class Gradient(Bandit):
    ...
    def update_est_value(self, action, step):
        ...
        self.est_values[action] = temp + \
            adj_reward*(1.0 - softmax[action].item())
